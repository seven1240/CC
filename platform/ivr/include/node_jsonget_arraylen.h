...
#ifndef _NODE_JSONGET_ARRAYLEN_H_
#define _NODE_JSONGET_ARRAYLEN_H_

#include "node_base.h"

class NodeJsonGetArrayLen : public NodeBase {
public:
    NodeJsonGetArrayLen(const uint32_t id, const string& name,
                        const string& type, const string& desc, const key_map_t& keymap);
    virtual ~NodeJsonGetArrayLen(void);
...
private:
    ...
    string _len;            ///< node中存在array len的变量名

    const static char* PARAM_INPUT;
    const static char* PARAM_KEY;
    const static char* PARAM_LEN;

...
namespace sql
{
    namespace mysql
    {
        ...
        public:
            ...
            MyVal(long double d) : val_type(typeDouble) {
                val.dval = d;
            }

            MyVal(double d) : val_type(typeDouble) {
                val.dval = d;
            }

            MyVal(int64_t l) : val_type(typeInt) {
                val.lval = l;
            }

            MyVal(uint64_t ul) : val_type(typeUInt) {
                val.ulval = ul;
            }

            MyVal(bool b) : val_type(typeBool) {
                val.bval = b;
            }

            MyVal(void* p) : val_type(typePtr) {
                val.pval = p;
            }
            ...
            const MyVal& operator=(const MyVal& rhs) {
                copy_obj(rhs);
                return *this;
            }

...
public:
    ...
    int32_t dial(const char* uuid, const char* called, const char* caller, uint32_t timeout,
                 bool external);
    ...
    int32_t bridgeex(const char* uuid, const char* caller, const char* called, bool external,
                     bool usebgm, const char* bgmfile);
    int32_t bridgeonly(const char* uuid1, const char* uuid2);

    int32_t startdtmf(const char* uuid);
    int32_t stopdtmf(const char* uuid);
    int32_t collect(const char* uuid, const char* playfile, uint32_t min, uint32_t max,
                    uint32_t timeout, const char* term);
    int32_t play_and_get_digits(const char* uuid, const char* playfile, uint32_t min, uint32_t max,
                                uint32_t tries,
                                uint32_t timeout, const char* term, const char* invalidfile);

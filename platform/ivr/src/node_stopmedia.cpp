...
NodeStopMedia::NodeStopMedia(const uint32_t id, const string& name,
                             const string& type, const string& desc, const key_map_t& keymap)
    : NodeResource(id, name, type, desc, keymap) {
}

NodeStopMedia::~NodeStopMedia(void) {
}

NodeBase* NodeStopMedia::run(base_script_t* param) {
    if (NULL == param) {
    ...
    if (true == parse_expression(_callid, param->name_var_map, callid)) {
        ...
        if (fs_mgr_t::instance()->fetch_opr(fs_no, &opr) == IVR_SUCCESS) {
            if (NULL != opr) {
                if (opr->stop_media(callid.c_str()) == IVR_SUCCESS) {
                    exit = EXIT_SUCC;
                } else {
                    IVR_WARN("opr stopmedia failed(callid %s)", _callid.c_str());
                }
